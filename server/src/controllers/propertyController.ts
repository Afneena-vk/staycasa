...
export class PropertyController implements IPropertyController {
  ...
  async updateOwnerProperty(
    ...
    try {
      ...
      const result = await this._propertyService.updateOwnerProperty(
        ownerId,
        propertyId,
        propertyData,
      );
  ...
  async checkAvailability(
    ...
    try {
      ...
      if (!checkIn || !rentalPeriod) {
        ...
        throw new AppError(
          "Check-in date and rental period are required",
      ...
      const result = await this._propertyService.checkAvailability(
        propertyId,
        checkIn as string,
        Number(rentalPeriod),
        Number(guests) || 1,
  ...
  async getDestinations(
    ...
    try {
      ...
      const destinations = await this._propertyService.getDestinations(
        search,
        page,
        limit,
      );
  ...
  async getLatestActiveProperties(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
      ...
      const response =
        await this._propertyService.getLatestActiveProperties(limit);
      res.status(response.status).json(response);
    } catch (error) {
      next(error);
    }
