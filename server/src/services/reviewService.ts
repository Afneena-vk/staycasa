...
export class ReviewService implements IReviewService {
  ...
  async createReview(
    ...
    const booking = await this._bookingRepository.findByBookingIdAndUser(
      bookingId,
      userId,
    );
    ...
    const existingReview =
      await this._reviewRepository.findByBookingId(bookingId);
    if (existingReview) {
    ...
    const { averageRating, totalReviews } =
      await this._reviewRepository.calculatePropertyRating(booking.propertyId);
  ...
  async getReviewsByPropertyId(
    ...
    return reviews.map((review) => ReviewMapper.toPropertyReviewDto(review));
  }
  ...
  async getReviewsByPropertyForAdmin(
    ...
    const reviews =
      await this._reviewRepository.findByPropertyIdForAdmin(propertyId);
    return reviews.map((review) => ReviewMapper.toPropertyReviewDto(review));
  }
  ...
  async toggleReviewVisibility(
    ...
    const review = await this._reviewRepository.toggleVisibility(
      reviewId,
      hide,
    );
    ...
    const { averageRating, totalReviews } =
      await this._reviewRepository.calculatePropertyRating(review.propertyId);
  ...
  async getReviewsByPropertyForOwner(
    ...
    const reviews = await this._reviewRepository.findByPropertyIdForOwner(
      propertyId,
      ownerId,
    );
